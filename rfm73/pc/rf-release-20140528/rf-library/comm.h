...
class COMM {
        public:
	//COMM(unsigned char id, void (*tmrcb)(union sigval si),user_activity *activity);
	COMM(unsigned char id, RFM70 *rfm, user_activity *activity);
        void send(const char *buf, unsigned char len);
	int connect(const char *buf, unsigned char len);
        void create(void);
